...
+  =:date= Group items that have a date associated. Argument can be =t= to match items with any date, =nil= to match items without a date, or =today= to match items with today's date.
+  =:deadline= Group items that have a deadline. Argument can be =t= (to match items with any deadline), =nil= (to match items that have no deadline), or =today= to match items whose deadline is today.
...
+  =:scheduled= Group items that are scheduled.  Group items that are scheduled. Argument can be =t= (to match items with any scheduled time), =nil= (to match items that have no scheduled time), or =today= to match items that are scheduled for today.
