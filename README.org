...
+  ~:auto-map~ :: This automatically groups items by the value returned when applying each item to the given function as a string from the agenda buffer ([[examples.org#automatically-by-mapping-a-function][example]]).  The function should return a string to be used as the grouping key and as the header for its group.  Some [[#auto-map-helper-functions][helper functions]] are provided.
...
These functions can be passed to the ~:auto-map~ selector to easily group items by certain attributes:

+  ~org-super-agenda--dir-name~ :: Return the item's directory name, prepended with =Directory:=, or nil if none.
+  ~org-super-agenda--parent-heading~ :: Return the item's parent heading, or nil if none.
...
+  Helper functions for the ~:auto-map~ selector: ~org-super-agenda--dir-name~ and ~org-super-agenda--parent-heading~.
